...
The model integrated with the whole Bert model, which is used to predict the masked amino acid label, fails to learn. We have already tested the MLP integration strategy mentioned earlier, which combines both the plain GVP-GNN and the Bert prediction. Therefore, we can rule out any fallacy in this approach. However, this failure could be due to the incompatibility of the representations used. The representations employed by the BERT LLM and the GVP-GNN models may not be compatible or could have different levels of abstraction. If the representations from the BERT LLM model and the GVP-GNN model are not effectively combined, it could result in limited or no improvement in performance. We have observed that incorporating the Bert model into the GVP-GNN hinders the model's learning process.
<ul>
